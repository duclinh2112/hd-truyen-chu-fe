...
export const Pagination: React.FC<PaginationProps> = ({
  ...
  const borderHoverClass = `${
    disabled ? '!cursor-not-allowed' : 'hover:border-primary hover:text-primary'
  }`
      ...
      const dots = (position: string) => (
        ...
        >
          ...
          <span className='hidden group-hover:block'>
            {position === 'left' ? (
              <IconDoubleArrowLeft className='text-primary' />
            ) : (
              <IconDoubleArrowRight className='text-primary' />
            )}
          </span>
        </li>
      )
      ...
      {showQuickJumper && (
        <div className='mx-1 flex items-center whitespace-nowrap font-medium text-primary'>
          ...
          <input
            ...
            type='number'
            className={twMerge(
              'mx-2 w-[50px] rounded-lg border border-border-200 bg-white px-2 py-1 font-normal',
              borderHoverClass,
              disabled ? 'bg-gray-50' : '',
            )}
            disabled={disabled}
