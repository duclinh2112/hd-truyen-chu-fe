...
export interface InputProps
  ...
  onChange?: (
    value: string,
    option?: React.ChangeEvent<HTMLInputElement>,
  ) => void
...
export const Input: React.FC<InputProps> = ({
  ...
  return (
    ...
    >
      {label && (
        <label className='mb-1 block font-medium text-main'>
          {label} {required && <span className='text-red-600'>*</span>}{' '}
          {optional && <span className='text-sm text-main'>(Optional)</span>}
        </label>
      )}
      <div
        ...
        >
          ...
          <div className='relative flex size-full items-center'>
            ...
            {loading && (
              <Spinner
                indicator={
                  <IconHalfCircleLoad width={16} className='fill-gray-500' />
                }
              />
            )}
          </div>
      ...
      {hintText ? <span className='mt-1 block text-main'>{hintText}</span> : ''}
