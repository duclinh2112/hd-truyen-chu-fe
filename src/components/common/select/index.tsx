import type { FC, Key, MouseEvent, ReactNode, Ref } from 'react'
import React, { useEffect, useRef } from 'react'
import { useState } from 'react'
import { createPortal } from 'react-dom'

import IconCheck from '@/components/icons/check'
import IconCloseX from '@/components/icons/close-x'
import IconDownArrow from '@/components/icons/down-arrow'
import IconHalfCircleLoad from '@/components/icons/half-circle-load'
...
export type SelectValueChangeProps =
  | string
  | number
...
export interface SelectProps {
  ...
  value?: string[] | string
  onChange?: (
    val: SelectValueProps,
    option?: OptionsProps | OptionsProps[],
  ) => void
  /**
  ...
  filterOption?:
    | boolean
    | ((options: OptionsProps[], searchKey: string) => OptionsProps[])
...
export const Select: SelectComponent = (props) => {
  ...
  const placeholderValue = placeholder || 'Chọn'
  ...
  const [labels, setLabels] = useState<string | React.ReactNode>(
    placeholderValue,
  )
  const [arrValueState, setArrValueState] = useState<string[]>(
  ...
  const onSelect = (e: MouseEvent, option: OptionsProps) => {
    e.stopPropagation()
  ...
  const onCancelSelect = (e: MouseEvent, value: string) => {
    e.stopPropagation()
    ...
    return (
      <div className='w-full px-1'>
        {showSearch ? (
          <input
            ref={inputRef}
            className={`bg-bg-secondary text-main ${
  ...
  const renderMultipleSelect = () => {
    const arr =
      options && options.filter((i) => (arrValueState || []).includes(i.value))
    return (
      <>
        {arr && arr.length > 0
          ? arr.map((item: OptionsProps) => {
              return renderMultiple ? (
                ...
                >
                  ...
                  <span
                    onClick={(e) =>
                      !disabled && !isLoading && onCancelSelect(e, item.value)
                    }
                    style={{
                      cursor: disabled || isLoading ? 'not-allowed' : 'pointer',
                    }}
                  >
                    <IconCloseX color='var(--text-main)' />
                  </span>
                </div>
              )
            })
        ...
        {showSearch ? (
          <input
            ...
            size={10}
            placeholder={
              arrValueState && arrValueState.length > 0 ? '' : placeholderValue
            }
            onClick={(e) => {
              const check = !disabled && !isLoading
              check && setVisible(true)
        ...
        {arrValueState && arrValueState.length === 0 && !showSearch ? (
          <span
            style={{ opacity: multiple ? 0.7 : 1 }}
            className='px-1 text-main'
          >
            {labels}
          </span>
        ) : null}
  ...
  const renderSuffixIcon = () => {
    if (isLoading)
      return <Spinner indicator={<IconHalfCircleLoad width={18} />} />
    const isClear =
      !disabled &&
      allowClear &&
      ((arrValueState && arrValueState.length > 0) || !!valueState)
    return (
      <>
        <span
          className={`${isClear && 'group-hover/icon:block'} hidden `}
        ...
        <div
          className={`flex h-full items-center justify-center ${
            isClear && 'group-hover/icon:hidden'
          } `}
        >
          {suffixIcon || (
            <span className='flex w-[18px] justify-center'>
              <IconDownArrow width={14} color='var(--primary)' />
            </span>
          )}
        </div>
      </>
  ...
  const renderChildren = () => {
    if (children?.length === 0)
      return renderEmpty ? (
        ...
        <Empty description={descriptionEmpty} src={imageEmpty} />
      )
      ...
      children.map((child) => {
        ...
        const selected =
          valueState === props.value
            ? 'bg-bg-100 font-medium cursor-pointer'
            : ''
        const normal =
          valueState !== props.value && !props.disabled
            ? 'hover:bg-bg-100 cursor-pointer transition duration-100 ease-in-out'
            : ''
  ...
  const renderOption = () => {
    ...
    if (typeof filterOption === 'boolean' && options) {
      if (filterOption) {
        const filter = options.filter((item) =>
          item.label.toString().toLowerCase().includes(searchKey.toLowerCase()),
        )
        arrMap = [...filter]
      } else {
        arrMap = [...options]
    ...
    return arrMap
      ? arrMap.map((opt) => {
          const firstOption = options?.find((i) =>
            (arrValueState || []).includes(i.value),
          )
          ...
          const createLabel = `Tạo "${searchKey}"`
          const searchSelectedCss =
            options && (searchKey === opt.label || createLabel === opt.label)
              ? 'bg-primary-200'
              : ''
          return (
            <li
            ...
            >
              {renderOptions ? (
                renderOptions(opt.label)
              ) : (
                <span className='truncate text-main'>{opt.label}</span>
              )}
              {selected && multiple ? (
                <IconCheck width={18} color='var(--text-main)' />
              ) : null}
            </li>
          )
        })
  ...
  const normalClass =
    !isLoading && !disabled ? 'cursor-pointer hover:border-primary' : ''
  const disabledClass =
    disabled || isLoading ? 'text-gray-500 !bg-bg-100 cursor-not-allowed' : ''
  const errorClass = validateErrors ? 'border-red' : 'border-border-200'
  ...
  return (
    <div
      ...
      className={`group/icon bg-bg-secondary relative flex items-center rounded-lg border px-2 py-1 text-sm ${errorClass} ${normalClass} ${disabledClass} ${className}`}
    >
      ...
      {register && (
        <input value={valueState} className='size-0' {...register} />
      )}
    </div>
  )
}
